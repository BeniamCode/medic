defmodule MedicWeb.DoctorLive.BookingComponent do
            ...
            <div class="card bg-primary/10 mt-6">
              <div class="card-body p-4">
                ...
                <div class="flex justify-between text-sm mb-2">
                  ...
                  <span class="font-medium">
                    <%= format_time(@selected_slot.starts_at) %> - <%= format_time(
                ...
                >
                  <%= if @booking do %>
                    Booking...
                  <% else %>
                    <.icon name="hero-check" class="w-4 h-4" /> Confirm Appointment
                  <% end %>
                </button>
              </div>
            </div>
  ...
  def handle_event("prev_week", _, socket) do
    ...
    new_start =
      if Date.compare(new_start, Date.utc_today()) == :lt do
        Date.utc_today()
      else
        new_start
      end
    ...
    case Scheduling.book_slot(
      ...
      {:error, :slot_already_booked} ->
        socket =
          ...
          |> assign(
            booking: false,
            booking_error: "This slot was just booked. Please choose another time."
          )
          |> load_week_slots()
