  ...
  return (
    <div style={{ padding: 16, paddingBottom: 80 }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, margin: '0 0 4px' }}>{t('Account settings')}</h2>
        <p style={{ color: '#666', margin: 0, fontSize: 14 }}>{t('Manage your profile and preferences')}</p>
      </div>

      <MobileCard title={t('Profile')} style={{ borderRadius: 12, marginBottom: 16 }}>
        <List>
      ...
      <MobileCard title={t('Security')} style={{ borderRadius: 12 }}>
        <MobileButton
          block
          disabled
          fill="outline"
...
function DesktopSettingsPage({ profile }: { profile: PageProps['profile'] }) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={2}>{t('Account settings')}</Title>
        <Text type="secondary">{t('Manage your profile and preferences')}</Text>
      </div>
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <DesktopCard bordered style={{ borderRadius: 12 }}>
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Text type="secondary" style={{ display: 'block', marginBottom: 4 }}>{t('Email')}</Text>
                <Text>{profile.email}</Text>
              </Col>
              <Col span={12}>
                <Text type="secondary" style={{ display: 'block', marginBottom: 4 }}>{t('Role')}</Text>
                <Text>{profile.role}</Text>
              </Col>
            </Row>
            <DesktopButton disabled>{t('Update profile')}</DesktopButton>
          </Space>
        </DesktopCard>

        <DesktopCard bordered style={{ borderRadius: 12 }}>
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            <Text strong>{t('Security')}</Text>
